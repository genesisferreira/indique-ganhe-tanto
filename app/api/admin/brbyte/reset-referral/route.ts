...
import { resetBrbyteReferralIntegration } from "@/lib/brbyte/reset-referral.service"
...
export async function POST(request: NextRequest) {
  const auth = await getAuthorizedBrbyteAdmin({
  ...
  const result = await resetBrbyteReferralIntegration({
    referralId,
